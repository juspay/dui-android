...
//        AxisLogger.d(LOG_TAG, "Default http client");
    ...
    public static byte[] get(String url, Map<String, String> queryParameters) {
        try {
//            AxisLogger.axisDebug(LOG_TAG, "Appending query parameters: " + queryParameters.toString());
...
//                    AxisLogger.d(LOG_TAG, "GZIP Header Present");
                    ...
                    try {
                        ...
                        if (zis != null) {
//                            AxisLogger.d(LOG_TAG,"CLOSING GZIP STREAM");
                            zis.close();
                        }
                    }
                } else {
...
//        AxisLogger.axisDebug(LOG_TAG, "Executing " + request.getMethod() + " " + request.getURI());
        try {
            request.setHeader("Accept-Encoding", "gzip");
            HttpResponse response = httpClient.execute(request);
//            AxisLogger.axisDebug(LOG_TAG, "Got response for " + request.getURI()
...
//            throw new RuntimeException(e.getMessage());
    ...
    public static byte[] gzipContent(byte[] uncompressed) {
        ...
        try {
            ...
            os.close();
            gzos.close();
//            AxisLogger.d(LOG_TAG, "Gzipping complete");
            return os.toByteArray();
        } catch (IOException ex) {
//            AxisLogger.trackAndLogException(LOG_TAG, "Could not gzip", ex);
...
//            AxisLogger.trackAndLogError(LOG_TAG,"Error While Downloading. Response Code:  "+response.getStatusLine().getStatusCode());
            return null;
        }
    }
